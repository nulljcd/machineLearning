class Network {
  ...
  constructor(layerSizes, activation, outputActivation, cost) {
    ...
    for (let l = 1; l < this.numLayers; l++) {
      ...
      for (let k = 0; k < this.layerSizes[l - 1]; k++) {
        ...
        for (let j = 0; j < this.layerSizes[l]; j++)
          array1[j] = (Math.sqrt(-2 * Math.log(1 - Math.random())) * Math.cos(2 * Math.PI * Math.random())) / Math.sqrt(this.layerSizes[l - 1]);
        array0.push(array1);
      }
